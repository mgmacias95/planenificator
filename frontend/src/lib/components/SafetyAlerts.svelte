	...
	{:else}
		<div class="max-h-[380px] space-y-2.5 overflow-y-auto pr-1">
			...
			{#each calculationStore.semicircularNotices as notice (notice.segmentIndex)}
				<div
					class="space-y-1.5 rounded-lg border border-l-4 border-slate-800 border-l-amber-500 bg-slate-900/90 p-3 text-xs shadow-xs"
				>
				...
				>
					<div class="flex items-center justify-between gap-2">
						<div class="flex items-center gap-1.5 font-bold text-white">
							...
							<span class="font-sans text-xs text-slate-400">({notam.location})</span>
						</div>
